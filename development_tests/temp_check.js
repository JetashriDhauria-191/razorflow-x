  ...
  async function sendMainChatMessage() {
    ...
    logTelemetry(`Customer prompt: "${message}"`, "#38bdf8");
  ...
  function handleQuickSelectProduct(id, name, price, img, cat) {
    ...
    logTelemetry(`Product selected: "${name}" (₹${price.toLocaleString('en-IN')})`, "#38bdf8");
  ...
  async function loadPreviousOrdersHistory() {
    ...
    try {
      ...
      if ($("historyOrderCountLabel")) $("historyOrderCountLabel").textContent = history.length;

      if (!history.length) {
        list.innerHTML = `<div style="text-align:center; padding:30px; color:#94a3b8;">No previous orders found.</div>`;
        return;
      }

      list.innerHTML = history.map(o => `
        <div style="background:#080c14; border:1px solid #1e293b; border-radius:10px; padding:10px 14px; display:flex; justify-content:space-between; align-items:center;">
          <div style="display:flex; gap:10px; align-items:center;">
            <img src="${(o.items && o.items[0] && o.items[0].image_url) || 'https://images.unsplash.com/photo-1505740420928-5e560c06d30e?w=200'}" 
                 style="width:44px; height:44px; object-fit:cover; border-radius:8px; background:#1e293b;">
            <div>
              <div style="font-size:12px; font-weight:800; color:white;">${(o.items && o.items[0] && o.items[0].name) || 'Shopping Order'}</div>
              <div style="font-size:10px; color:#94a3b8;">Order ID: <code style="color:#38bdf8;">${o.order_id}</code> · Tx: <code>${o.payment_id.slice(0, 12)}...</code></div>
              <div style="font-size:10px; color:#a7f3d0;">${o.delivery_speed || '⚡ 1-Day Express SLA'} · ${o.delivery_date || 'Delivered'}</div>
            </div>
          </div>
          <div style="text-align:right;">
            <strong style="color:#4ade80; font-size:14px; font-family:var(--font-mono);">₹${Number(o.total_amount).toLocaleString('en-IN')}</strong><br>
            <span class="badge-tag badge-green" style="font-size:9px; padding:2px 6px;">${o.status}</span>
          </div>
        </div>
      `).join("");
    } catch (e) {
  ...
  async function loadCart() {
    ...
    try {
      ...
      const renderItemHtml = (item) => {
        ...
        const delDate = item.estimated_delivery_date || (delDays === 1 ? "Tomorrow (Wed, 2 Sep)" : `Sat, 5 Sep`);
        const badgeClass = delDays === 1 ? "express" : (delDays <= 2 ? "standard" : "regional");

        return `
          <div class="cart-item-card" style="background:#080c14; border:1px solid #1e293b; border-radius:10px; padding:10px 12px; margin-bottom:8px;">
            <div class="cart-item-main-row" style="display:flex; gap:10px; align-items:center; margin-bottom:6px;">
              <img src="${item.image_url || 'https://images.unsplash.com/photo-1526170375885-4d8ecf77b99f?w=200'}" 
                   alt="${item.name}" 
                   style="width:48px; height:48px; object-fit:cover; border-radius:8px; background:#1e293b; flex-shrink:0;">
              <div style="flex:1; min-width:0;">
                <h4 style="font-size:12px; font-weight:700; color:#f8fafc; margin-bottom:2px; line-height:1.3;" title="${item.name}">${item.name}</h4>
                <div style="display:flex; justify-content:space-between; align-items:center;">
                  <span style="color:#4ade80; font-size:13px; font-weight:900; font-family:var(--font-mono);">₹${Number(item.line_total || (item.price * item.quantity)).toLocaleString("en-IN")}</span>
                  <small style="color:#94a3b8; font-size:10px;">(₹${Number(item.price).toLocaleString("en-IN")} each)</small>
                </div>
              </div>
              <div style="display:flex; align-items:center; gap:4px; flex-shrink:0;">
                <button class="btn-hands-free" style="padding:2px 6px; font-weight:800;" onclick="updateCartQty('${pId}', ${item.quantity - 1})">-</button>
                <span style="font-weight:800; font-size:12px; min-width:16px; text-align:center;">${item.quantity}</span>
                <button class="btn-hands-free" style="padding:2px 6px; font-weight:800;" onclick="updateCartQty('${pId}', ${item.quantity + 1})">+</button>
                <button class="btn-test-cards" style="padding:4px 6px; color:#ef4444;" onclick="removeFromCart('${pId}')" title="Remove">🗑️</button>
              </div>
            </div>

            <div style="display:flex; justify-content:space-between; align-items:center; border-top:1px dashed #1e293b; padding-top:6px;">
              <span class="cart-item-delivery-badge ${badgeClass}" style="font-size:10px; padding:2px 6px; border-radius:4px;">
                ${delSpeed}
              </span>
              <span style="font-size:10px; color:#cbd5e1;">
                Arriving: <strong style="color:white;">${delDate}</strong>
              </span>
            </div>
          </div>
        `;
      ...
      if (list) {
        list.innerHTML = cleanItems.length ? cleanItems.map(renderItemHtml).join("") : `<div style="text-align:center; padding:30px; color:#94a3b8;">Your cart is empty.</div>`;
      }
      if (flowContainer) {
      ...
      if ($("cartSubtotalDisplay")) $("cartSubtotalDisplay").textContent = `₹${Number(subtotalVal).toLocaleString("en-IN")}`;
      if ($("cartSavingsDisplay")) $("cartSavingsDisplay").textContent = `-₹${Number(savingsVal).toLocaleString("en-IN")}`;
      if ($("cartTotalDisplay")) $("cartTotalDisplay").textContent = `₹${Number(totalVal).toLocaleString("en-IN")}`;

      if ($("flowSubtotalDisplay")) $("flowSubtotalDisplay").textContent = `₹${Number(subtotalVal).toLocaleString("en-IN")}`;
      if ($("flowSavingsDisplay")) $("flowSavingsDisplay").textContent = `-₹${Number(savingsVal).toLocaleString("en-IN")}`;
      if ($("flowTotalDisplay")) $("flowTotalDisplay").textContent = `₹${Number(totalVal).toLocaleString("en-IN")}`;
      if ($("gateOrderTotalDisplay")) $("gateOrderTotalDisplay").textContent = `Order Total: ₹${Number(totalVal).toLocaleString("en-IN")}`;
      if ($("deskPriceDisplay")) $("deskPriceDisplay").textContent = `₹${Number(totalVal).toLocaleString("en-IN")}`;
